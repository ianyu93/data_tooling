...
def extract_opening_text(
    source, n_docs: int = 10_000, accent=False, case=True, numbers=True, punct=1
):
    i = 0
    for doc in jsonql.read_jsons(source):
        ...
        yield text_normalizer.normalize(
            text, accent=accent, case=case, numbers=numbers, punct=punct
        )
        i += 1
        if i >= n_docs:
            break
...
def wget(url: str, output: Path):
    subprocess.run(
        ["wget", url, "-O", tmp(output), "--no-check-certificate"], check=True
    )
    tmp(output).replace(output)
    assert (
